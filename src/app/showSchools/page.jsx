...
export default function ShowSchools() {
  ...
  if (loading) {
    return <div className="text-center py-20 text-gray-700">Loading schools...</div>;
  }

  return (
    <div className="min-h-screen bg-gradient-to-b text-gray-500 from-blue-100 to-white py-10 px-4">
        ...
        {filteredSchools.length === 0 && !error ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {filteredSchools.map((school) => {
              const publicId = school.image.split('/').slice(-2).join('/').split('.')[0];
              return (
                <div
                  key={school.id}
