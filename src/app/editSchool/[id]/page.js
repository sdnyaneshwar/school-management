  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setTimeout(() => router.push('/showSchools'), 1000); // Redirect after 1s
      } else {
  ...
  return (
    <div className="min-h-screen text-gray-400 bg-gradient-to-b from-blue-100 to-white flex flex-col items-center py-10 px-4">
      <div className="bg-white p-8 rounded-xl shadow-lg w-full max-w-lg">
        <h1 className="text-3xl font-bold text-gray-800 mb-6 text-center">Edit School</h1>
        <form onSubmit={handleSubmit(onSubmit)} encType="multipart/form-data">
          ...
          <div className="mb-5">
            <label className="block text-sm font-medium text-gray-700 mb-1">School Image (Optional)</label>
            <input
              type="file"
              {...register('image')}
